...
def GetDataSourcePais():
    #pathData="C:/Users/LESLY/Documents/GitHub/practica-python-datux/Proyecto/files/data.xls"
...
def GetDatasourceOrders(conn):
    ...
    df_orders=df[['Order ID','Postal Code','Product ID','Sales','Quantity','Discount','Profit','Shipping Cost','Order Priority', 'Order Date']].dropna().drop_duplicates()
    ...
    print("Columnas en df_newOrders:", df_newOrders.columns)

    df_newOrders=df_newOrders[['Order ID','Postal Code','id','Sales','Quantity','Discount','Profit','Shipping Cost','Order Priority', 'Order Date']]
...
def insertManyVentas(bd:Database,data):
    bd.insert_many('VENTAS',['order_id','postal_code','product_id','sales_amount','quantity','discount','profit','shipping_cost','order_priority', 'order_date'],data)